...
//Description of changes: I've ensured that the tree will not include
//    duplicates of items (by modifying insert) as we were told in class to
//    avoid duplicate words. Added a new find method that will return the
//    number of comparisons made in the search.
...
int avlTree::find(string key, string &type) {
	node *nodePtr = root;
	int comparisons = 0;
	//Find the key
	while (nodePtr) {
...
avlTree::node* avlTree::remove(node*& root, string key)
{
	if (!root)
	{
		return NULL;
...
avlTree::node* avlTree::predSuccessor(node *root)
{
	node *current = root;
